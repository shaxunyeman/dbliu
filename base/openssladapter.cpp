...
BIO* BIO_new_socket(base::AsyncSocket* socket) {
...
static int socket_read(BIO* b, char* out, int outl) {
	if (!out)
		return -1;
	base::AsyncSocket* socket = static_cast<base::AsyncSocket*>(b->ptr);
...
ssl_error:
	...
	int OpenSSLAdapter::Recv(void* pv, size_t cb) {
	...
	bool OpenSSLAdapter::VerifyServerName(SSL* ssl, const char* host, bool ignore_bad_cert) {
		if (!host)
			return false;
